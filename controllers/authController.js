...
const loginVoter = async (req, res) => {
  try {
    ...
    const voter = await Voter.findOne({ cnic }).select('+password');
    if (!voter) {
      return res.status(401).json(errorResponse('Invalid credentials', 401));
    }
    ...
    if (!voter.isActive) {
      return res.status(401).json(errorResponse('Your account has been deactivated', 401));
    }
